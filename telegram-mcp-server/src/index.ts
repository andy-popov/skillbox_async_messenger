 ...
 */
function createShutdown(cleanup: () => Promise<void> | void) {
...
function installLifecycleHandlers(shutdown: (reason: string, exitCode?: number) => void): void {
 ...
 */
function watchParentProcess(shutdown: (reason: string, exitCode?: number) => void): void {
...
async function runHTTP(): Promise<void> {
  ...
  httpServer.on("error", (error: NodeJS.ErrnoException) => {
    if (error.code === "EADDRINUSE") {
      console.error(
        `ERROR: port ${port} is already in use — a previous telegram-mcp-server instance is probably still running.\n` +
          `Stop it first:\n` +
