...
function respondWithMessage(message: TelegramMessage, format: ResponseFormat) {
  const text =
...
export function registerMessageTools(server: McpServer): void {
  ...
  const SendMessageSchema = z
    .object({
      ...
      reply_to_message_id: MessageIdSchema.optional().describe(
        "If set, sends this message as a reply to the given message ID"
      ),
...
Error Handling:
  ...
  - Returns "Error: Bad request" if chat_id does not exist or parse_mode formatting is invalid`,
    ...
    async (params: z.infer<typeof SendMessageSchema>) => {
      try {
...
Error Handling:
  - Returns "Error: Bad request" if the message is too old to edit, unchanged, or was not sent by this bot`,
    ...
    async (params: z.infer<typeof EditMessageTextSchema>) => {
      try {
        const message = await callTelegramApi<TelegramMessage>("editMessageText", {
          chat_id: params.chat_id,
...
Error Handling:
  ...
  - Returns "Error: Forbidden" if the bot lacks delete rights (needs admin "can_delete_messages" in groups/channels)`,
    ...
    async (params: z.infer<typeof DeleteMessageSchema>) => {
      try {
        await callTelegramApi<boolean>("deleteMessage", params);
        return {
          content: [
            {
              type: "text" as const,
...
Don't use when: you want to resend content without attribution — copy the text into telegram_send_message instead.`,
    ...
    async (params: z.infer<typeof ForwardMessageSchema>) => {
      try {
        const message = await callTelegramApi<TelegramMessage>("forwardMessage", params);
        return respondWithMessage(message, params.response_format);
      } catch (error) {
        return { content: [{ type: "text" as const, text: formatTelegramError(error) }] };
      }
...
Don't use when: you can just send the message immediately — this is a UX nicety, not required.`,
    ...
    async (params: z.infer<typeof SendChatActionSchema>) => {
      try {
        await callTelegramApi<boolean>("sendChatAction", params);
        return { content: [{ type: "text" as const, text: `Sent '${params.action}' action to chat ${params.chat_id}.` }] };
      } catch (error) {
        return { content: [{ type: "text" as const, text: formatTelegramError(error) }] };
      }
    }
...
Error Handling:
  - Returns "Error: Bad request" if the URL is unreachable or not a valid image, or the file_id is invalid/expired`,
    ...
    async (params: z.infer<typeof SendPhotoSchema>) => {
      try {
        const message = await callTelegramApi<TelegramMessage>("sendPhoto", {
          chat_id: params.chat_id,
          photo: params.photo,
...
Don't use when: sending a photo meant to display inline (use telegram_send_photo instead — documents are sent as downloadable files).`,
    ...
    async (params: z.infer<typeof SendDocumentSchema>) => {
      try {
        const message = await callTelegramApi<TelegramMessage>("sendDocument", {
          chat_id: params.chat_id,
...
Error Handling:
  - Returns "Error: Forbidden" if the bot is not an admin or lacks pin rights`,
    ...
    async (params: z.infer<typeof PinMessageSchema>) => {
      try {
        await callTelegramApi<boolean>("pinChatMessage", params);
        return { content: [{ type: "text" as const, text: `Pinned message ${params.message_id} in chat ${params.chat_id}.` }] };
      } catch (error) {
        return { content: [{ type: "text" as const, text: formatTelegramError(error) }] };
      }
    }
...
Error Handling:
  - Returns "Error: Forbidden" if the bot is not an admin or lacks pin rights`,
    ...
    async (params: z.infer<typeof UnpinMessageSchema>) => {
      try {
        await callTelegramApi<boolean>("unpinChatMessage", params);
        return { content: [{ type: "text" as const, text: `Unpinned message in chat ${params.chat_id}.` }] };
      } catch (error) {
        return { content: [{ type: "text" as const, text: formatTelegramError(error) }] };
      }
    }
