...
export function truncateJson(payload: unknown): string {
...
export function formatUserLine(user: TelegramUser): string {
  const name = [user.first_name, user.last_name].filter(Boolean).join(" ");
  const handle = user.username ? ` (@${user.username})` : "";
  return `${name}${handle} [id: ${user.id}]`;
}

export function formatChatLine(chat: TelegramChat): string {
...
export function formatMessageMarkdown(message: TelegramMessage): string {
  const lines = [
    `## Message ${message.message_id}`,
    `- **Chat**: ${formatChatLine(message.chat)}`,
  ];
  if (message.from) lines.push(`- **From**: ${formatUserLine(message.from)}`);
  lines.push(`- **Date**: ${new Date(message.date * 1000).toISOString()}`);
  if (message.text) lines.push(`- **Text**: ${message.text}`);
  if (message.caption) lines.push(`- **Caption**: ${message.caption}`);
  return lines.join("\n");
}
