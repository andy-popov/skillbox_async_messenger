...
export function registerUpdateTools(server) {
    ...
    const GetUpdatesSchema = z
        ...
        offset: z
            ...
            .int()
            .optional()
            .describe("Only return updates with update_id >= offset. Pass (last_seen_update_id + 1) to fetch new updates and mark earlier ones as read; omit to see the oldest pending updates."),
  ...
  - Returns "Error: Bad request" if a webhook is currently set (delete it first via BotFather or the setWebhook API before polling)`,
        ...
        try {
            ...
            if (params.response_format === ResponseFormat.JSON) {
                return { content: [{ type: "text", text: truncateJson(output) }], structuredContent: output };
            }
            if (!updates.length) {
                return { content: [{ type: "text", text: "No new updates." }], structuredContent: output };
            }
            ...
            for (const update of updates) {
                ...
                if (msg) {
                    ...
                    lines.push(`- [${update.update_id}] ${kind} in ${formatChatLine(msg.chat)}${msg.from ? ` from ${formatUserLine(msg.from)}` : ""}: "${msg.text ?? msg.caption ?? "(no text)"}" (message_id: ${msg.message_id})`);
                }
                else if (update.callback_query) {
                    lines.push(`- [${update.update_id}] callback_query "${update.callback_query.data ?? ""}" from ${formatUserLine(update.callback_query.from)} (callback_query_id: ${update.callback_query.id})`);
                }
                else {
    ...
    const AnswerCallbackQuerySchema = z
        ...
        .strict();
    server.registerTool("telegram_answer_callback_query", {
...
Use when: the bot has an inline keyboard and just received a callback_query update from telegram_get_updates.`,
        ...
        try {
            ...
            return { content: [{ type: "text", text: `Answered callback query ${params.callback_query_id}.` }] };
        }
        catch (error) {
            return { content: [{ type: "text", text: formatTelegramError(error) }] };
        }
