...
export class TelegramApiError extends Error {
    errorCode;
    retryAfter;
    constructor(message, errorCode, retryAfter) {
        super(message);
...
export async function callTelegramApi(method, params) {
    ...
    try {
        ...
        if (!response.data.ok) {
            throw new TelegramApiError(response.data.description || "Unknown Telegram API error", response.data.error_code, response.data.parameters?.retry_after);
        }
        ...
        throw new TelegramApiError(`Unexpected error calling Telegram API: ${error instanceof Error ? error.message : String(error)}`);
    }
}
function telegramErrorFromAxios(error) {
    const body = error.response?.data;
    if (body && typeof body === "object" && "description" in body) {
        return new TelegramApiError(body.description || "Telegram API request failed", body.error_code, body.parameters?.retry_after);
    }
    if (error.code === "ECONNABORTED") {
        return new TelegramApiError("Request to Telegram API timed out. Please try again.");
    }
    return new TelegramApiError(`Telegram API request failed: ${error.message}`);
}
...
export function formatTelegramError(error) {
    if (error instanceof TelegramApiError) {
        switch (error.errorCode) {
            ...
            case 409:
                return (`Error: Conflict — ${error.message}. Telegram allows only one getUpdates poller per bot token: ` +
